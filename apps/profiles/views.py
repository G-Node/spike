...
def profile(request, username, template_name="profiles/profile.html", extra_context=None):
    ...
    blist = Benchmark.objects.filter(owner=other_user).filter(state="A")
    elist = Evaluation.objects.filter(owner=other_user, publication_state=1)
    if request.user == other_user:
        # add private benchmarks
        blist = blist | Benchmark.objects.filter(owner=other_user, state="N")
        # add private evaluations
        elist = elist | Evaluation.objects.filter(owner=other_user, publication_state=0) |\
            Evaluation.objects.filter(owner=other_user, publication_state=2)
    ...
    return render_to_response(template_name, dict({
        ...
        "experiments": experiments, # user experiments
        "blist": blist, # user benchmarks
        "elist": elist, # user evaluations
    }, **extra_context), context_instance=RequestContext(request))
